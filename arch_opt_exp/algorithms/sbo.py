...
class MinVariancePFInfill(FunctionEstimatePoFInfill):
    ...
    def select_infill_solutions(self, population: Population, infill_problem: Problem, n_infill) -> Population:
        ...
        Default.tcv.do(pop_var, inplace=True)
...
class SBOInfill(InfillCriterion):
    ...
    @property
    def name(self):
        return '%s / %s' % (self._surrogate_model_base.__class__.__name__, self.infill.__class__.__name__)
        ...
        if termination is None or not isinstance(termination, Termination):
            ...
            if n_obj > 1:
                termination = MultiObjectiveDefaultTermination(
                    x_tol=5e-4, cv_tol=1e-8, f_tol=5e-3, nth_gen=5, n_last=robust_period, n_max_gen=n_max_gen,
                    n_max_evals=n_max_eval)
            else:
                termination = SingleObjectiveDefaultTermination(
                    x_tol=1e-8, cv_tol=1e-8, f_tol=1e-6, n_last=robust_period, n_max_gen=n_max_gen, n_max_evals=n_max_eval)
